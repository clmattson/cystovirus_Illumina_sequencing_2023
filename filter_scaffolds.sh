...
for file in *${sample_suffix}_output/*${sample_suffix}_M_scaffolds.fasta; 
  ...
  awk 'BEGIN{FS="\n";RS=">";ORS=""} (NR==FNR)&&(NR>1){headers[$1];next} ($1 in headers){print ">" $0}' *${sample_suffix}_output/${fileonly}_covlenfilt_names.txt *${sample_suffix}_output/${sample}${sample_suffix}_M_scaffolds.fasta >> *${sample_suffix}_output/${sample}${sample_suffix}_M_covlenfilt_scaffolds.fasta;
  echo "kcov values above 30% or 300 = ${filtered_cov_values[@]} + length values above 20% or 800 = ${filtered_len_values[@]}";
  echo;
  echo "${sample},$(grep -c '>' *${sample_suffix}_output/${sample}${sample_suffix}_M_covlenfilt_scaffolds.fasta),${filtered_len_values[0]},${filtered_len_values[-1]},${filtered_cov_values[0]},${filtered_cov_values[-1]}" >> M_all_scaffold_stats.csv;
done
